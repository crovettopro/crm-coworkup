...
export default async function OccupancyPage({
  ...
  const [{ data: clients }, { data: subs }, { data: rentedExtras }] = await Promise.all([
    supabase.from("clients").select("id, status, coworking_id").in("coworking_id", cwIds),
    supabase.from("subscriptions").select("plan_name, coworking_id, status").in("coworking_id", cwIds).eq("status", "active"),
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <MetricCard label="Clientes activos" value={active} tone="brand" />
